    ...
    <Card className="w-full max-w-250 overflow-hidden">
      <CardHeader className="grid gap-4 md:grid-cols-[1fr_auto] md:items-start">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-border-default bg-surface-canvas px-4 py-3 text-content-secondary text-sm">
            Shared tokens
          </div>
          <div className="rounded-xl border border-border-default bg-surface-canvas px-4 py-3 text-content-secondary text-sm">
            Shared primitives
      ...
      <CardContent className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
        ...
        <div className="rounded-xl border border-border-default bg-surface-canvas p-5 text-content-secondary text-sm leading-6">
          所有应用都通过同一套共享组件输出界面，后续调整主题或组件状态时，只需要维护一处。
        </div>
      </CardContent>

      <CardFooter className="flex-wrap gap-3 border-border-default border-t pt-6">
        <Button asChild variant="ghost">
          <a href="/design-system" rel="noopener noreferrer" target="_blank">
            View design system
          </a>
        </Button>
      </CardFooter>
